...
The DreamLedger Core daemon can be configured to act as a trusted "border
router", implementing the dreamledger wire protocol and relay, making
...
The ZeroMQ feature in DreamLedger Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building dreamledgerd:
...
For instance:
    ...
    $ dreamledgerd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/dreamledgerd.tx.raw \
...
These options can also be provided in dreamledger.conf.
...
From the perspective of dreamledgerd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into dreamledgerd directly. Furthermore, no information is
...
using. DreamLedgerd appends an up-counting sequence number to each
