...
const char * const BITCOIN_CONF_FILENAME = "dreamledger.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //dreamledger-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\DreamLedger
    // macOS: ~/Library/Application Support/DreamLedger
    // Unix-like: ~/.dreamledger
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DreamLedger";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/DreamLedger";
#else
    // Unix-like
    return pathRet / ".dreamledger";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty dreamledger.conf if it does not exist
