...
impl PrivateKey {
    ...
    pub fn generate_dreamledger(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "dreamledger_helpers")]
    pub fn generate_dreamledger_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_dreamledger(rng))
            .collect()
    }
